...
impl Finger {
	...
	pub fn length(&self) -> f32 {
		let proximal_position: Vec3A = self.proximal.pose.position.into();
		let distal_position: Vec3A = self.distal.pose.position.into();
		let tip_position: Vec3A = self.tip.pose.position.into();

		proximal_position.distance(distal_position) + distal_position.distance(tip_position)
	}

	pub fn direction(&self) -> mint::Vector3<f32> {
		let proximal_position: Vec3A = self.proximal.pose.position.into();
		let tip_position: Vec3A = self.tip.pose.position.into();
...
impl Hand {
	...
	pub fn palm_normal(&self) -> mint::Vector3<f32> {
		(Quat::from(self.palm.pose.orientation) * vec3a(0.0, -1.0, 0.0)).into()
	}

	/// The direction vector pointing from the palm to thumb.
	pub fn radial_axis(&self) -> mint::Vector3<f32> {
		(Quat::from(self.palm.pose.orientation)
	...
	pub fn distal_axis(&self) -> mint::Vector3<f32> {
		(Quat::from(self.palm.pose.orientation) * vec3a(0.0, 0.0, -1.0)).into()
	}
	...
	pub fn pinch_distance(&self, finger: &Finger) -> f32 {
		let thumb_tip: Vec3A = self.thumb.tip.pose.position.into();
		let finger_tip: Vec3A = finger.tip.pose.position.into();
		thumb_tip.distance(finger_tip)
	}

	/// Unstabilized pinch position.
	pub fn pinch_position(&self) -> mint::Vector3<f32> {
		let thumb_tip: Vec3A = self.thumb.tip.pose.position.into();
		let index_tip: Vec3A = self.index.tip.pose.position.into();
	...
	pub fn stable_pinch_position(&self) -> mint::Vector3<f32> {
		let index_knuckle: Vec3A = self.index.proximal.pose.position.into();
	...
	pub fn predicted_pinch_position(&self) -> mint::Vector3<f32> {
		let thumb_tip: Vec3A = self.thumb.tip.pose.position.into();
		let index_tip: Vec3A = self.index.tip.pose.position.into();
		let index_knuckle: Vec3A = self.index.proximal.pose.position.into();
		let index_length = self.index.length();
	...
	fn hand_scale(&self) -> f32 {
		let index_metacarpal: Vec3A = self.index.metacarpal.pose.position.into();
		let index_proximal: Vec3A = self.index.proximal.pose.position.into();

		let middle_metacarpal: Vec3A = self.middle.metacarpal.pose.position.into();
		let middle_proximal: Vec3A = self.middle.proximal.pose.position.into();

		let ring_metacarpal: Vec3A = self.ring.metacarpal.pose.position.into();
		let ring_proximal: Vec3A = self.ring.proximal.pose.position.into();

		let little_metacarpal: Vec3A = self.little.metacarpal.pose.position.into();
		let little_proximal: Vec3A = self.little.proximal.pose.position.into();
	...
	pub fn pinch_strength(&self) -> f32 {
		let thumb_tip: Vec3A = self.thumb.tip.pose.position.into();
		let index_tip: Vec3A = self.index.tip.pose.position.into();
		let middle_tip: Vec3A = self.middle.tip.pose.position.into();
		let ring_tip: Vec3A = self.ring.tip.pose.position.into();
		let little_tip: Vec3A = self.little.tip.pose.position.into();
...
impl Pointer {
	pub fn direction(&self) -> mint::Vector3<f32> {
		(Quat::from(self.pose.orientation) * vec3a(0.0, 0.0, -1.0)).into()
	}
}
