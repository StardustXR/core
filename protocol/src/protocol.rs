pub mod server {
	...
	impl Server {
		...
		pub fn from_object_or_ref(obj: binderbinder::binder_object::BinderObjectOrRef) -> Server {
...
pub mod spatial {
	...
	pub const EXTERNAL_PROTOCOL: gluon_wire::ExternalGluonProtocol =
		gluon_wire::ExternalGluonProtocol {
			protocol_name: "org.stardustxr.Spatial.gluon",
			types: &[
	...
	impl gluon_wire::GluonConvertable for Quat {
		fn write<'a, 'b: 'a>(
			&'b self,
			data: &mut gluon_wire::GluonDataBuilder<'a>,
		) -> Result<(), gluon_wire::GluonWriteError> {
	...
	impl Spatial {
		...
		pub fn set_parent(&self, parent: SpatialRef) -> Result<(), gluon_wire::GluonSendError> {
			let mut builder = gluon_wire::GluonDataBuilder::new();
		...
		///only use this when you know the binder ref implements this interface, else the consquences are for you to find out
		pub fn from_object_or_ref(obj: binderbinder::binder_object::BinderObjectOrRef) -> Spatial {
			let drop_notification = obj
				.device()
				.register_object(gluon_wire::drop_tracking::DropNotifiedHandler::new());
			let mut builder = gluon_wire::GluonDataBuilder::new();
				...
				match transaction_code {
					...
					10u32 => {
						let (bounding_box) = self
							.get_world_bounding_box(ctx, gluon_wire::GluonConvertable::read(data)?)
							.await;
						bounding_box.write_owned(&mut out)?;
					}
					11u32 => {
						let (transform) = self
							.get_relative_transform(ctx, gluon_wire::GluonConvertable::read(data)?)
							.await;
		...
		fn dispatch_one_way(
			...
			async move {
				match transaction_code {
					...
					13u32 => {
						self.set_parent_in_place(ctx, gluon_wire::GluonConvertable::read(data)?);
					}
					_ => {}
				}
				Ok(())
	...
	impl SpatialInterface {
		...
		pub async fn create_spatial(&self) -> Result<SpatialRef, gluon_wire::GluonSendError> {
			let obj = binderbinder::binder_object::ToBinderObjectOrRef::to_binder_object_or_ref(
				&self.obj,
			);
			tokio::task::spawn_blocking(move || {
		...
		pub fn create_spatial_blocking(&self) -> Result<SpatialRef, gluon_wire::GluonSendError> {
			let mut builder = gluon_wire::GluonDataBuilder::new();
			let reader = self
				.obj
				.device()
	...
	pub trait SpatialInterfaceHandler:
		binderbinder::device::TransactionHandler + Send + Sync + 'static
	{
		///Create a new spatial object.
		fn create_spatial(
