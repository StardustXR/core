...
pub mod drawable {
    ...
    pub struct DmatexSubmitInfo {
        ///The id of the Dmatex to be used
        pub dmatex_id: u64,
        ///the point the timeline reaches once the client is done accessing the texture
...
pub mod camera {
    ...
    impl crate::node::NodeType for Camera {
        fn node(&self) -> &crate::node::NodeCore {
            &self.core
        }
    }
    ...
    impl SpatialAspect for Camera {}
    impl OwnedAspect for Camera {}
    impl SpatialRefAspect for Camera {}
    impl CameraAspect for Camera {}
    #[derive(Debug)]
    pub enum CameraEvent {}
    ///
    pub trait CameraAspect: crate::node::NodeType + super::SpatialAspect + super::OwnedAspect + super::SpatialRefAspect + std::fmt::Debug {
        ...
        fn request_draw(
            ...
            let data = (
                render_target,
                views.iter().map(|a| Ok(a)).collect::<crate::node::NodeResult<Vec<_>>>()?,
            );
            self.node()
                .send_signal(9815139681665817635u64, 16461261915714313182u64, &data)?;
            let (render_target, views) = data;
            tracing::trace!(
                ? render_target, ? views, "Sent signal to server, {}::{}", "Camera",
                "request_draw"
            );
            Ok(())
        }
    ...
    pub fn create_camera(
        ...
        parent: &impl SpatialRefAspect,
        transform: Transform,
    ) -> crate::node::NodeResult<Camera> {
        {
            let data = (id, parent.node().id, transform);
            let (serialized_data, fds) = stardust_xr_wire::flex::serialize(&data)?;
            _client
                .message_sender_handle
                .signal(11u64, 0u64, 886756885299557230u64, &serialized_data, fds)?;
            let (id, parent, transform) = data;
            tracing::trace!(
                ? id, ? parent, ? transform, "Sent signal to server, {}::{}",
                "Interface", "create_camera"
            );
        }
        Ok(Camera::from_id(_client, id, true))
    }
}
#[allow(unused_imports)]
use item_panel::*;
