...
async fn fusion_environment_ui() {
	...
	struct EnvironmentUIManager(Arc<Client>, rustc_hash::FxHashMap<String, EnvironmentItem>);
	impl crate::items::ItemUIHandler<EnvironmentItem> for EnvironmentUIManager {
		fn created(&mut self, uid: &str, item: EnvironmentItem, path: String) {
			self.1.insert(uid.to_string(), item);
			println!("Environment item {uid} created with path {path}");
		}
		fn captured(&mut self, uid: &str, acceptor_uid: &str, _item: EnvironmentItem) {
			println!("Capturing environment item {uid} in acceptor {acceptor_uid}");
		}
		fn released(&mut self, uid: &str, acceptor_uid: &str, _item: EnvironmentItem) {
	...
	let _item_ui = crate::items::ItemUI::register(&client)
		.unwrap()
		.wrap(EnvironmentUIManager(client.clone(), FxHashMap::default()))
		.unwrap();
